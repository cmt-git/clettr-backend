    ...
    Query: {
        user: async (parent, args, context) => {
            ...
            if (context.user !== undefined){
                ...
                if (args.username != undefined && args.username !== null && args.username !== "" && args.username.length <= 16){
                    const query_user = await userEntity.findOne({where:{username: args.username}});
        ...
        user_info: async (parent, args, context) => {
            if (context.user !== undefined){
                ...
                .where("user_id.bsc_address = :value", {value: context.user.bsc_address})
                .getOne();
                ...
                json_template.username_change_time = current_user_info.username_change_time;
                ...
                .where("nft_entity.nft_type = :value and nft_entity.current_owner = :id and (nft_entity.status is null or nft_entity.status != 'burned')", {value: "active", id: context.user.id})
                .getRawOne();
                ...
                .where("nft_entity.nft_type = :value and nft_entity.current_owner = :id and (nft_entity.status is null or nft_entity.status != 'burned')", {value: "passive", id: context.user.id})
                .getRawOne();
                ...
                .select(`COUNT(*)`, "total_active_nfts")
                .where("current_owner.bsc_address = :value and nft_entity.nft_type = 'active'", {value: context.user.bsc_address, })
                .getRawOne();
                ...
                if (current_user_info.refill_energy){
                    ...
                    .set({ current_energy: json_template.max_energy, refill_energy: false})
                    .where("user_info_entity.user_id = :value", {value: context.user.id})
                    .execute();
                }
