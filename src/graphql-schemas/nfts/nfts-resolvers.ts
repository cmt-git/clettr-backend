    ...
    Query: {
        owned_nfts: async (parent, args, context) => {
            if (context.user != null){
                ...
                let user_type = args.not_user !== true ? "(current_owner = :value or current_owner.username = :value)" : "(current_owner != :value)";
                ...
                if (args.filters == null){
                    ...
                    .where(`${user_type} and (nft_entity.status != 'burned' or nft_entity.status is null)`, {value: args.username != null ? args.username : context.user.id})
                    ...
                    const filter_stars = args.filters.split("-").length > 1 ? `and (nft_entity.nft_stars = ${args.filters.split("-")[1]})` : "";

                    if (filter_type == "active"){ //? -> this is a temporary patch
                        query = await getConnection()
                        .getRepository(nftEntity)
                        .createQueryBuilder("nft_entity")
                        .leftJoinAndSelect("nft_entity.current_owner", "current_owner")
                        .leftJoinAndSelect("nft_entity.original_owner", "original_owner")
                        .where(`${user_type} and (nft_entity.status != 'burned' or nft_entity.status is null) and (nft_entity.nft_type = 'active') ${filter_stars}`, {value: args.username != null ? args.username : context.user.id})
                        .offset(!args.page ? 0 : (args.page - 1) * 10)
                        .limit(30)
                        .orderBy("nft_entity.id", "DESC")
                        .getMany();
                    ...
                    if (filter_type == "passive"){ //? -> this is a temporary patch
                        query = await getConnection()
                        .getRepository(nftEntity)
                        .createQueryBuilder("nft_entity")
                        .leftJoinAndSelect("nft_entity.current_owner", "current_owner")
                        .leftJoinAndSelect("nft_entity.original_owner", "original_owner")
                        .where(`${user_type} and (nft_entity.status != 'burned' or nft_entity.status is null) and (nft_entity.nft_type = 'passive') ${filter_stars}`, {value: args.username != null ? args.username : context.user.id})
                        .offset(!args.page ? 0 : (args.page - 1) * 10)
                        .limit(30)
                        .orderBy("nft_entity.id", "DESC")
                        .getMany();
                ...
                for(let i = 0; i < query.length; i++){
                    nft_arr.push({
                        ...
                        nft_long_id: query[i].nft_long_id,
                        ...
                        nft_requirement: query[i].nft_requirement != null ? query[i].nft_requirement.toString() : "",
                        status: query[i].status != null ? query[i].status.toString() : "",
                        market_info: query[i].market_info != null? query[i].market_info.toString() : ""
                    });
                }
                    ...
                    if (i > 0 && keys[i].split("_")[0] == "set"){
                        if (query[keys[i]] != null){
                            nft_arr.push({
                                ...
                                nft_long_id: query[keys[i]].nft_long_id,
                                ...
                                nft_requirement: query[keys[i]].nft_requirement != null ? query[keys[i]].nft_requirement.toString() : "",
                                status: query[keys[i]].status != null ? query[keys[i]].status.toString() : "",
                                market_info: query[keys[i]].market_info != null? query[keys[i]].market_info.toString() : ""
                            });
                        }
                    }
                }
        ...
        market_nfts: async (parent, args, context) => {
            ...
            .where("nft_entity.nft_type = :value and nft_entity.status = 'market_sell'", {value: "active"})
            .getRawOne();
            ...
            .where("nft_entity.nft_type = :value and nft_entity.status = 'market_sell'", {value: "passive"})
            .getRawOne();
