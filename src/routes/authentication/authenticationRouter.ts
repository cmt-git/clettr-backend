...
export function randomAuthCode() {
...
export const generateAuthenticationCode = async (json: any, res: any) => {
  const authentication_code = randomAuthCode();
  const checkAuthentication = await authenticationEntity.findOne({
    where: { email: json.email },
  });
  const checkSignature = await userEntity.findOne({
    where: { bsc_address: json["bsc_address"] },
  });
  ...
  if (checkAuthentication === undefined && checkSignature !== undefined) {
    ...
    await sendMail(json["email"], "Authentication Code", "auth-code", {
      authentication_code: authentication_code,
    });

    return res.status(200).send({
      message:
        "Authentication Code has been sent. Please use within 10 minutes.",
      success: true,
    });
  } else {
...
export const generateLongAuthCode = async (json: any, res: any) => {
  ...
  for (let i = 0; i < 50; i++) {
    authentication_code +=
      characters[Math.floor(Math.random() * characters.length)].toString();
  }
  ...
  await authenticationEntity
    .create({
      email: json.email,
      long_authentication_code: authentication_code,
    })
    .save();

  await sendMail(json["email"], "Authentication Code", "email-code", {
    authentication_code: authentication_code,
...
export const cleanAuthenticationEntity = async () => {
  ...
  await getConnection()
    ...
    .where("time_registered < :subtractedDate", {
